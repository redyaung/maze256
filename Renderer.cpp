...
Renderer::Renderer(LedControl compassDisp)
  : compassDisp(compassDisp) {}

void Renderer::renderWideVision(Vision<5> vision) {
...
void Renderer::renderCompass(float degree) {
  ...
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      ...
      compassDisp.setLed(0, row, col, painted);
    }
  }
  previous = d;
}
